...
import { projectActivity, projectArtifacts, projectMilestones, projects, reflections } from "../db/schema.js";
import {
  addProjectArtifact,
  addProjectMilestone,
  createProject,
...
class PostgreSqlWorkspaceRepository implements WorkspaceRepository {
  ...
  public async getPersonalDashboard(userId: string): Promise<ReturnType<typeof getPersonalDashboard>> {
    const [projectRows, milestoneRows, artifactRows, reflectionRows, activityRows] = await Promise.all([
      this.listProjects(userId),
      this.database.select({ id: projectMilestones.id, status: projectMilestones.status }).from(projectMilestones).innerJoin(projects, eq(projectMilestones.projectId, projects.id)).where(eq(projects.ownerId, userId)),
      this.database.select({ id: projectArtifacts.id }).from(projectArtifacts).innerJoin(projects, eq(projectArtifacts.projectId, projects.id)).where(eq(projects.ownerId, userId)),
      this.database.select({ id: reflections.id }).from(reflections).where(eq(reflections.userId, userId)),
      this.database.select().from(projectActivity).innerJoin(projects, eq(projectActivity.projectId, projects.id)).where(eq(projects.ownerId, userId))
    ]);
    const mappedProjects = projectRows.map((row) => toProject(row).project);
    const recentActivity = activityRows
      .map(({ project_activity: activity }) => toActivity(activity))
  ...
  public async getProjectDetail(userId: string, projectId: string): Promise<WorkspaceProjectDetail | null> {
    ...
    const [milestoneRows, artifactRows, activityRows] = await Promise.all([
      this.database.select().from(projectMilestones).where(eq(projectMilestones.projectId, projectId)),
      this.database.select().from(projectArtifacts).where(eq(projectArtifacts.projectId, projectId)),
  ...
  public async addArtifact(userId: string, projectId: string, input: { title: string; kind: ProjectArtifactKind; note?: string }): Promise<ProjectArtifact | null> {
    ...
    if (!project) return null;
    const now = new Date();
    const [row] = await this.database.insert(projectArtifacts).values({ projectId, title: input.title, kind: input.kind, note: input.note || null, createdAt: now }).returning();
  ...
  private async listProjects(userId: string) {
    return this.database.select().from(projects).where(eq(projects.ownerId, userId));
  }
...
function toArtifact(row: typeof projectArtifacts.$inferSelect): ProjectArtifact {
  return { id: row.id, projectId: row.projectId, title: row.title, kind: row.kind as ProjectArtifactKind, note: row.note, createdAt: iso(row.createdAt) };
}
