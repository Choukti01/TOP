...
import { getCollaborationInvitations, getIncomingConnectionRequests, getTopNotifications, markTopNotificationRead, respondToCollaborationInvitation, respondToConnectionRequest, subscribeToTopSignals, type CollaborationInvitation, type ConnectionRequest, type TopNotification } from "../../lib/api";
...
async function load(): Promise<void> {
  loading.value = true;
  try {
    const [invitationResult, connectionResult, notificationResult] = await Promise.all([getCollaborationInvitations(), getIncomingConnectionRequests(), getTopNotifications()]);
...
.invitation-list li.highlighted { animation:invitation-arrival 1.8s ease-in-out 2; background:rgba(98,230,255,.11); border-left-color:var(--top-cyan); box-shadow:0 0 0 1px rgba(98,230,255,.22),0 0 30px rgba(98,230,255,.1); scroll-margin:30px; } @keyframes invitation-arrival { 50% { transform:translateX(4px); } }
</style>
