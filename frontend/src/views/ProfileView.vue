    ...
    <main class="profile-shell">
      ...
      <section class="profile-card">
        ...
        <form v-else-if="activePanel === 'field'" class="profile-form" @submit.prevent="saveField">
          <h2>Set the tone of your field.</h2><p>These private settings shape how TOP welcomes you back. You can change them whenever life changes.</p>
          ...
          <div class="profile-actions"><button class="profile-save" type="submit" :disabled="fieldName.length < 2">Save field settings <span>↗</span></button><button class="profile-leave" type="button" @click="router.push('/workspace')">Enter my field</button></div>
        </form>

        <section v-else class="profile-account">
          <h2>Your account, kept quiet.</h2><p>TOP does not sell your attention or make your profile public by default.</p><div class="account-row"><span>EMAIL</span><strong>{{ authState.user?.email }}</strong></div><div class="account-row"><span>SESSION</span><strong>Secure on this browser</strong></div><div class="account-row"><span>PROFILE</span><strong>Private by default</strong></div><div class="account-note"><i>✓</i><p>When database hosting is connected, your account, projects, and evidence persist safely across devices.</p></div><div class="profile-actions"><button class="profile-save" type="button" @click="router.push('/workspace')">Return to my work <span>↗</span></button><button class="profile-leave" type="button" @click="signOut">Sign out</button></div></section>
      </section>
    </main>
  </section>
</template>
...
import { getPersonalDashboard, type PersonalDashboard } from "../lib/api";
...
const activePanel = ref<"overview" | "identity" | "field" | "account">("overview");
const fieldName = ref("");
...
const panelHeading = computed(() => ({ overview: "YOUR TOP SIGNAL", identity: "IDENTITY, NOT PERFORMANCE", field: "YOUR PRIVATE FIELD", account: "PRIVACY & ACCESS" })[activePanel.value]);
...
async function signOut(): Promise<void> { await leaveTop(); await router.push("/"); }
...
async function chooseAvatar(event: Event): Promise<void> {
  ...
  isError.value = false;
  message.value = "Profile photo applied to your TOP field.";
}
...
function openProject(projectId: string): void {
  void router.push({ path: "/workspace", query: { project: projectId } });
}
...
function saveField(): void {
  ...
  try {
    ...
    fieldMessage.value = "This browser could not save field settings locally.";
  }
}
</script>
