  ...
  <section class="join-view">
    ...
    <main class="join-shell">
      ...
      <section class="join-card" aria-live="polite">
        ...
        <template v-if="googleAvailable && !resetToken && !recoveryMode">
          <div class="join-divider"><span>or</span></div>
          <button class="join-google" type="button" :disabled="submitting" @click="startGoogle"><b>G</b> Continue with Google</button>
        </template>
...
import { createTopAccount, restoreTopSession, signInToTop, verifyTopEmail } from "../lib/auth";
import { getTopAuthProviders, getTopGoogleSignInUrl, requestTopPasswordReset, resetTopPassword } from "../lib/api";
...
function safeNext(): string {
  return typeof route.query.next === "string" && route.query.next.startsWith("/") && !route.query.next.startsWith("//") ? route.query.next : "/top";
}

function startGoogle(): void {
  error.value = "";
  try { window.location.assign(getTopGoogleSignInUrl(safeNext())); }
...
async function submit(): Promise<void> {
  ...
  try {
    ...
    const next = safeNext();
    await router.push(user.emailVerified ? next : { path: "/verify-email", query: { next } });
  } catch (reason) {
...
onMounted(async () => {
  try { googleAvailable.value = (await getTopAuthProviders()).google; } catch { googleAvailable.value = false; }
  const google = typeof route.query.google === "string" ? route.query.google : "";
  if (google === "success") {
    submitting.value = true;
    try {
      const user = await restoreTopSession();
      if (!user) throw new Error("Google sign-in ended before TOP could restore your secure session. Please try again.");
      await router.replace(route.query.fresh === "1" ? "/onboarding" : safeNext());
      return;
    } catch (reason) {
      error.value = reason instanceof Error ? reason.message : "Google sign-in could not be completed.";
    } finally { submitting.value = false; }
  } else if (google) {
    error.value = google === "denied" ? "Google sign-in was cancelled. You can choose email instead." : google === "unavailable" ? "Google sign-in is not configured for TOP yet." : "Google sign-in could not be completed. Please try again.";
  }
...
<style scoped>
.join-divider { align-items:center; color:var(--top-muted); display:flex; font-family:var(--top-mono); font-size:9px; gap:12px; letter-spacing:.12em; margin:24px 0 14px; text-transform:uppercase; }.join-divider::before,.join-divider::after { background:rgba(127,159,255,.2); content:""; flex:1; height:1px; }.join-google { align-items:center; background:rgba(5,8,24,.55); border:1px solid rgba(214,225,255,.3); border-radius:999px; color:var(--top-ink); cursor:pointer; display:flex; font:600 11px inherit; gap:10px; justify-content:center; padding:13px 18px; transition:.2s ease; width:100%; }.join-google:hover:not(:disabled) { border-color:var(--top-cyan); box-shadow:0 0 0 3px rgba(98,230,255,.08); transform:translateY(-2px); }.join-google:disabled { cursor:wait; opacity:.65; }.join-google b { align-items:center; background:#fff; border-radius:50%; color:#4285f4; display:inline-flex; font-family:Arial,sans-serif; font-size:13px; font-weight:700; height:19px; justify-content:center; width:19px; }
