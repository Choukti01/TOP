<template>
  <aside class="atlas-layer">
    <button class="brand" type="button" aria-label="Return to your private Field" @click="returnToUniverse">
    ...
    <button class="member-mark" type="button" aria-label="Open your TOP profile" @click="openProfile">
      ...
      <span><small>YOUR FIELD</small><strong>{{ authState.user?.fieldName || authState.user?.displayName || 'My profile' }}</strong></span>
      <b v-if="unreadSignalCount > 0" class="signal-badge" :aria-label="`${unreadSignalCount} unread collaboration notifications`">{{ unreadSignalCount > 9 ? '9+' : unreadSignalCount }}</b>
    </button>
        ...
        <section id="top-atlas" class="atlas" role="dialog" aria-modal="true" aria-label="TOP atlas navigation">
          <header>
            ...
            <div class="atlas-header-actions">
              <button class="atlas-sign-out" type="button" @click="signOut">Sign out securely</button>
          ...
          <nav aria-label="Private Field sections">
            <button
          ...
          <footer>
            ...
            <button v-if="WorkspaceState.activeSection !== 'Overview'" type="button" @click="returnToUniverse">Back to your Field</button>
            <button type="button" @click="signOut">Sign out securely</button>
          </footer>
        </section>
      </div>
    </Transition>
...
import { getTopNotifications, subscribeToTopSignals } from "../../lib/api";
...
const unreadSignalCount = ref(0);
const router = useRouter();
let signalTimer: number | undefined;
let stopLiveSignals: (() => void) | undefined;
...
const navigation: Array<{ name: WorkspaceSection; icon: string; kicker: string }> = [
  ...
  { name: "Signals", icon: "◌", kicker: "CIRCLE" },
  { name: "AI", icon: "✦", kicker: "CLARIFY" },
  { name: "Reflection", icon: "◒", kicker: "NOTICE" }
];

const activeTitle = computed(() => WorkspaceState.activeSection === "Overview" ? "Your private Field" : WorkspaceState.activeSection);
...
async function refreshSignalCount(): Promise<void> {
  try {
    const { notifications } = await getTopNotifications();
    unreadSignalCount.value = notifications.filter((notification) => !notification.readAt).length;
  } catch {
    unreadSignalCount.value = 0;
  }
}

onMounted(() => {
  window.addEventListener("keydown", closeOnEscape);
  void refreshSignalCount();
  signalTimer = window.setInterval(() => { void refreshSignalCount(); }, 30_000);
  stopLiveSignals = subscribeToTopSignals(() => { void refreshSignalCount(); });
});
onUnmounted(() => {
...
@media (max-width:700px) { .brand { left:17px; top:20px; }.brand-rule,.brand small { display:none; }.atlas-trigger { right:16px; top:17px; }.reflection-orbit { bottom:17px; right:16px; }.atlas-backdrop { align-items:flex-start; overflow:auto; padding:14px; }.atlas { border-radius:23px; max-height:none; min-height:calc(100dvh - 28px); padding:28px 22px 34px; width:calc(100vw - 28px); }.atlas nav { grid-template-columns:1fr; }.atlas nav button { min-height:52px; }.atlas h2 { font-size:36px; }.atlas footer { align-items:flex-start; flex-direction:column; margin-top:25px; padding-bottom:8px; }.atlas footer button { margin-left:0; }.atlas header .atlas-sign-out { padding:9px 10px; } }
...
.signal-badge { align-items:center; background:var(--top-pink); border:2px solid #080c1e; border-radius:999px; color:#07101d; display:flex; font-family:var(--top-mono); font-size:7px; font-weight:900; height:17px; justify-content:center; min-width:17px; padding:0 4px; position:absolute; right:-7px; top:-6px; } @media (max-width:700px) { .signal-badge { right:-6px; top:-5px; } }
</style>
